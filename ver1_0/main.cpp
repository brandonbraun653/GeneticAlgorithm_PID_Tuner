...
int trial_popSizes[MAX_TRIALS] = { 5, 10, 25, 50, 100, 250, 500 };
...
int main()
{
	...
	boost::container::vector<SS_NLTIV_ModelBase_sPtr> trial_eval_models(MAX_TRIALS);
	...
	for (int opNum = 0; opNum < NUM_OPTIMIZERS; opNum++)
	{
		...
		* GA Execution Steps
		*------------------------*/
		trial_steps.data()[opNum].breedType =				GA_BREED_SIMPLE_CROSSOVER;
		...
		trial_steps.data()[opNum].filterType =				GA_POPULATION_DYNAMIC_FILTER;
		trial_steps.data()[opNum].selectType =				GA_SELECT_TOURNAMENT;
